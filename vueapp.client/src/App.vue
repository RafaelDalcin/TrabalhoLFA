  ...
  export default defineComponent({
    ...
    methods: {
      ...
      criarAutomato() {
        ...
        Object.keys(this.gramatica.producoesPorNaoTerminal).forEach(de => {
          ...
          producoes.forEach(producao => {
            if (producao === 'ε') {
              // Se a produção é epsilon (ε), o estado de destino é um estado final
              ...
              // Verifique se a chave 'de' já está inicializada em transicoes
              if (!transicoes[de]) {
                transicoes[de] = [];  // Inicialize se necessário
              }

              // Adicionar transição de "de" para "destino" com o símbolo "simbolo"
              transicoes[de].push({ simbolo, destino });

              // Se a produção termina com um terminal, marca o estado como final
      ...
      verificarString() {
        ...
        let stringProcessada = 0;

        // Processar a string e verificar se é aceita pelo autômato
        for (let i = 0; i < this.stringEntrada.length; i++) {
          ...
          // Se não encontrar transição, rejeita a string
        ...
        // Verificar se o estado final foi alcançado após consumir todos os símbolos ou se há transição no final
        if (this.af.estadosFinais.includes(estadoAtual) && transicaoNoFinal) {
